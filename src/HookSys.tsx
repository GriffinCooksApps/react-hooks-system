...
//TODO : REview to idetnify any missing pieces that I need to pull over.

const HookFluxSystem = ({ onReady, children }) => {
	...
	React.useEffect(() => {
		if (dstate !== null && (state === null || state.version !== dstate.version)) {
			setState(dstate);
		}
	}, [state, dstate, setState]);

	const offSpring = state === null || !fonts ? <View /> : <ErrorHandler>{children}</ErrorHandler>;
	...
	return <HookContext.Provider value={{ state, dispatch }}>{offSpring}</HookContext.Provider>;
};
